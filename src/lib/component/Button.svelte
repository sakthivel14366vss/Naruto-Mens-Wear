<script>
	let { children, isDisabled = false, color = 'blue', type = 'button', ...props } = $props();

	const colorClassList = {
		blue: `bg-blue-500 text-white hover:bg-blue-600 focus:bg-blue-600 outline-blue-600 active:bg-blue-700
	...
	const colorClass = $derived(colorClassList[color]);
	const disabledClass = 'cursor-not-allowed bg-gray-400! !text-gray-300';
</script>

{#if isDisabled}
	<button
		type="button"
		class="inline-flex items-center justify-center rounded px-2 py-1 font-semibold text-white select-none {disabledClass}"
		disabled
		tabindex="-1"
	>
		{@render children()}
...
{:else}
	<button
		class="inline-flex items-center justify-center rounded px-2 py-1 font-semibold text-white select-none focus:outline-3 focus:outline-offset-2 {colorClass}"
		{type}
		{...props}
	>
		{@render children()}
